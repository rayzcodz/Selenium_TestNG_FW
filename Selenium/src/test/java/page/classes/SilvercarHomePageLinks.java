...
public class SilvercarHomePageLinks {
	...
	public SilvercarHomePageLinks(WebDriver driver) {
		this.driver = driver;
	...
	public List<WebElement> getAllActiveLinks() {
		...
		for (int i = 0; i < alllinks.size(); i++) {
			...
			if (alllinks.get(i).getAttribute("href") != null && (!alllinks.get(i).getAttribute("href").contains("javascript"))) {
				if (! alllinks.get(i).getAttribute("href").equals("tel:8553592227") &&
						! alllinks.get(i).getAttribute("href").equals("mailto:support@silvercar.com")) {
